...
def get_match_stats_html(username):
    ...
    for entry in reversed(stats_data["data"]):
        ...
        html += "<th>Name</th><th>Job</th><th>Pts</th><th>CatLoss</th><th>InjLoss</th><th>InjTot</th><th>InjPtTot</th><th>Avg</th></tr></thead><tbody>"
        ...
        for farmer in farmers:
            ...
            html += f"<tr><td>{name}</td><td>{job}</td><td>{pts}</td><td>{catlost} {cat_flag}</td><td>{injlost}</td><td>{injtot}</td><td>{injptot}</td><td>{avg:.2f}</td></tr>"
            match_total += pts
