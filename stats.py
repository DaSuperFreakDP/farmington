...
def get_match_stats_html(username):
    ...
    for entry in reversed(stats_data["data"]):
        ...
        for farmer in farmers:
            ...
            html += f"<tr><td>{name}</td><td>{job}</td><td>{total_pts}</td><td>{task_pts}</td><td>{crop_pts}</td><td>{catlost} {cat_flag}</td><td>{injlost}</td><td>{injtot}</td><td>{injptot}</td><td>{avg:.2f}</td></tr>"
    ...
    for username, user_data in data["users"].items():
        for entry in user_data.get("data", []):
            for farmer in entry["farmers"]:
                ...
                stats = farmer_stats[name]
                stats["total_points"] += farmer["points_after_catastrophe"]
                stats["matchdays"] += 1
                stats["total_injuries"] += farmer.get("injuries_this_season", 0)
                stats["total_injury_points"] += farmer.get("injury_points_lost", 0)
                stats["roles_played"].add(farmer["job"])
                stats["recent_points"].append(farmer["points_after_catastrophe"])
                
                # Keep only last 5 performances
                if len(stats["recent_points"]) > 5:
                    stats["recent_points"] = stats["recent_points"][-5:]
    
    # Convert sets to lists for JSON serialization
    for name, stats in farmer_stats.items():
        stats["roles_played"] = list(stats["roles_played"])
        stats["avg_points"] = stats["total_points"] / stats["matchdays"] if stats["matchdays"] > 0 else 0
        stats["recent_avg"] = sum(stats["recent_points"]) / len(stats["recent_points"]) if stats["recent_points"] else 0
