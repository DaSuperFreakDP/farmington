...
def main():
    ...
    def load_farmer_pool_for_league(league_code):
        if league_code:
            # Check if previous season stats exist
            prev_stats_file = f"previous_szn_stats_{league_code}.json"
            if os.path.exists(prev_stats_file):
                # Previous season exists, use league-specific evolved farmer pool
                league_pool_file = f"farmer_pool_{league_code}.json"
                try:
                    with open(league_pool_file, "r") as f:
                        print(f"[DEBUG] Using evolved farmer pool for league {league_code}")
                        return json.load(f)
                except FileNotFoundError:
                    print(f"[DEBUG] League farmer pool not found, falling back to original")
                    pass

        # No previous season or league code, use original farmer pool
        try:
            with open("farmer_pool.json", "r") as f:
                print(f"[DEBUG] Using original farmer pool")
                return json.load(f)
        except FileNotFoundError:
            return []
