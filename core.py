...
def main():
    if len(sys.argv) < 2:
        raise ValueError("Usage: python core.py <username> [league_code]")
    username = sys.argv[1]
    league_code = sys.argv[2] if len(sys.argv) > 2 else None
    print(f"[core.py] Running for user: {username}, League Code: {league_code}")
    ...
    if len(characters) == 0:
        ...
        # Get league-wide matchday instead of incrementing individual counter
        current_matchday = 1  # Default for non-league play
        if league_code:
            try:
                with open("leagues.json", "r") as f:
                    leagues = json.load(f)
                if league_code in leagues:
                    current_matchday = leagues[league_code].get("current_matchday", 1)
            except:
                current_matchday = 1
