...
def get_current_matchup(username, league):
    ...
    # Get user's current matchday to determine which 3-game cycle they're in
    user_data = get_user_stats(username)
    matchday = user_data.get("matchday", 0)
    
    # Each matchup lasts 3 matchdays
    cycle = matchday // 3
    
    # Create pairings for this cycle using round-robin style
    ...
    # Simple pairing: player i pairs with player (i + cycle) % (num_players - 1)
    # This ensures everyone gets different opponents across cycles
    if num_players <= 2:
        # With 2 players, they always face each other
        opponent_index = 1 - user_index
    else:
        # With more players, use round-robin rotation
        available_opponents = [i for i in range(num_players) if i != user_index]
        opponent_index = available_opponents[cycle % len(available_opponents)]
...
def update_playoff_records(league_code):
    ...
    # Helper function to get opponent for a given cycle
    def get_matchup_opponent(username, cycle_num):
        if username not in players:
            return None
        ...
        # Simple pairing: player i pairs with player (i + cycle + 1) % num_players
        opponent_index = (user_index + cycle_num + 1) % num_players
        
        # Avoid self-pairing
        if opponent_index == user_index:
            opponent_index = (opponent_index + 1) % num_players
    ...
    # Check each player's current cycle completion
    for player in players:
        user_data = get_user_stats(player)
        matchday = user_data.get("matchday", 0)
        
        # Only process if this player just completed a 3-game cycle
        if matchday > 0 and matchday % 3 == 0:
            current_cycle = matchday // 3 - 1  # The cycle that was just completed (0-indexed)
            
            # Get opponent for this cycle
            opponent = get_matchup_opponent(player, current_cycle)
            if not opponent:
                continue
            ...
            # Only record the matchup once (from the first player processed)
            # Check if opponent has also completed this cycle
            ...
            if user_data.get("drafted_team") and len(user_data["drafted_team"]) >= 3:
                ...
                try:
                    ...
                    if user_league:
                        ...
                        if user_league.get("use_playoffs", True):
                            update_playoff_records(user_league["code"])
