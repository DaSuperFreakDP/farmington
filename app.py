...
def get_league_current_matchday(league_code):
    """Get the current league-wide matchday"""
    leagues = load_leagues()
    if league_code not in leagues:
        return 0

    league = leagues[league_code]
    return league.get("current_matchday", 0)

def advance_league_matchday(league_code):
    """Advance the league-wide matchday counter"""
    leagues = load_leagues()
    if league_code not in leagues:
        return 0

    league = leagues[league_code]
    current_matchday = league.get("current_matchday", 0) + 1
    league["current_matchday"] = current_matchday
    leagues[league_code] = league
    save_leagues(leagues)
    return current_matchday

def get_current_matchup_pairs(league_code):
    """Get all current matchup pairs for the league based on league matchday"""
    leagues = load_leagues()
    if league_code not in leagues:
        return {}
    ...
    # Use league-wide matchday instead of individual player matchdays
    league_matchday = league.get("current_matchday", 0)
    cycle = league_matchday // 3

    # Check if we're in the bracket phase (past halfway point)
    halfway_point = league.get("matchdays", 30) // 2
    matchday_halfway = halfway_point // 3  # Convert to cycles

    matchups = {}

    if cycle >= matchday_halfway:
        # Bracket phase - only match within same bracket
        brackets = get_league_brackets(league_code)

        for bracket_name, bracket_players in brackets.items():
            if len(bracket_players) < 1:
                continue
            elif len(bracket_players) == 1:
                # Single player in bracket gets bye week
                matchups[bracket_players[0]] = None
                continue

            bracket_cycle = cycle - matchday_halfway

            # Create round-robin matchups for this bracket
            if len(bracket_players) == 2:
                matchups[bracket_players[0]] = bracket_players[1]
                matchups[bracket_players[1]] = bracket_players[0]
            else:
                # For larger brackets, create round-robin pairs with bye weeks for odd numbers
                for i, player in enumerate(bracket_players):
                    available_opponents = [p for j, p in enumerate(bracket_players) if j != i]

                    # If odd number of players, one gets a bye each cycle
                    if len(bracket_players) % 2 == 1:
                        bye_player_index = bracket_cycle % len(bracket_players)
                        if i == bye_player_index:
                            matchups[player] = None  # Bye week
                            continue

                        # Remove bye player from available opponents
                        bye_player = bracket_players[bye_player_index]
                        available_opponents = [p for p in available_opponents if p != bye_player]

                    if available_opponents:
                        opponent = available_opponents[bracket_cycle % len(available_opponents)]
                        matchups[player] = opponent
                    else:
                        matchups[player] = None

    else:
        # Regular season phase - match with anyone
        if len(players) == 1:
            # Single player gets bye week
            matchups[players[0]] = None
        elif len(players) == 2:
            matchups[players[0]] = players[1]
            matchups[players[1]] = players[0]
        else:
            # Create round-robin matchups for all players with bye weeks for odd numbers
            for i, player in enumerate(players):
                available_opponents = [p for j, p in enumerate(players) if j != i]

                # If odd number of players, one gets a bye each cycle
                if len(players) % 2 == 1:
                    bye_player_index = cycle % len(players)
                    if i == bye_player_index:
                        matchups[player] = None  # Bye week
                        continue

                    # Remove bye player from available opponents
                    bye_player = players[bye_player_index]
                    available_opponents = [p for p in available_opponents if p != bye_player]

                if available_opponents:
                    opponent = available_opponents[cycle % len(available_opponents)]
                    matchups[player] = opponent
                else:
                    matchups[player] = None
...
def get_current_matchup(username, league):
    ...
    matchups = get_current_matchup_pairs(league["code"])
    opponent = matchups.get(username)

    # Return "BYE WEEK" if no opponent is found
    return opponent if opponent else "BYE WEEK"
...
def get_matchup_progress(username, league):
    ...
    # Use league-wide matchday instead of individual player matchday
    league_matchday = league.get("current_matchday", 0)

    # Determine progress in current 3-game cycle
    games_in_cycle = league_matchday % 3
    return {
        "games_played": games_in_cycle,
        "games_remaining": 3 - games_in_cycle if games_in_cycle > 0 else 3
    }

def get_league_brackets(league_code):
    """Get the current brackets for a league"""
    leagues = load_leagues()
    if league_code not in leagues:
        return {"top": [], "bottom": []}
    ...
    # Check if brackets are already calculated
    if "brackets" in league:
        return league["brackets"]

    # Calculate brackets based on current win records
    playoff_records = league.get("playoff_records", {})
    players = league.get("players", [])

    # Sort players by wins (descending)
    sorted_players = sorted(players, key=lambda p: playoff_records.get(p, {}).get("wins", 0), reverse=True)

    # Split into top and bottom halves
    mid_point = len(sorted_players) // 2
    top_bracket = sorted_players[:mid_point] if mid_point > 0 else sorted_players
    bottom_bracket = sorted_players[mid_point:] if mid_point > 0 else []

    # Ensure each bracket has at least one player
    if not top_bracket and bottom_bracket:
        top_bracket = [bottom_bracket.pop(0)]
    elif not bottom_bracket and top_bracket:
        if len(top_bracket) > 1:
            bottom_bracket = [top_bracket.pop()]
...
def update_league_brackets(league_code):
    """Update and save league brackets at the halfway point"""
    leagues = load_leagues()
    if league_code not in leagues:
        return
    ...
    # Check if we're at the halfway point using league-wide matchday
    halfway_point = league.get("matchdays", 30) // 2
    league_matchday = league.get("current_matchday", 0)

    # If we've reached the halfway point and brackets aren't set
    if league_matchday >= halfway_point and "brackets" not in league:
        brackets = get_league_brackets(league_code)
        league["brackets"] = brackets
        leagues[league_code] = league
        save_leagues(leagues)
        logging.info(f"Created brackets for league {league_code}: Top {len(brackets['top'])}, Bottom {len(brackets['bottom'])}")

    return league.get("brackets", {"top": [], "bottom": []})
...
def update_playoff_records(league_code):
    ...
    if "playoff_records" not in league:
        league["playoff_records"] = {player: {"wins": 0, "losses": 0, "ties": 0} for player in players}

    # Track which matchups have been recorded to avoid duplicates
    if "recorded_matchups" not in league:
        league["recorded_matchups"] = []

    # Track bye weeks that have been processed
    if "recorded_byes" not in league:
        league["recorded_byes"] = []

    # Use league-wide matchday instead of individual player matchdays
    league_matchday = league.get("current_matchday", 0)
    max_cycles = league_matchday // 3

    # Helper function to get opponent for a given cycle using league matchday system
    def get_matchup_opponent_for_cycle(username, cycle_num):
        if username not in players:
            return None

        # Check if we're in bracket phase
        halfway_point = league.get("matchdays", 30) // 2
        matchday_halfway = halfway_point // 3  # Convert to cycles

        if cycle_num >= matchday_halfway and "brackets" in league:
            # Bracket phase - find opponent within same bracket
            brackets = league["brackets"]
            user_bracket = None

            for bracket_players in brackets.values():
                if username in bracket_players:
                    user_bracket = bracket_players
                    break

            if not user_bracket or len(user_bracket) < 2:
                return None

            user_index = user_bracket.index(username)
            bracket_size = len(user_bracket)
            bracket_cycle = cycle_num - matchday_halfway

            if bracket_size <= 2:
                opponent_index = 1 - user_index
            else:
                available_opponents = [i for i in range(bracket_size) if i != user_index]
                if len(available_opponents) == 0:
                    return None  # Bye week
                opponent_index = available_opponents[bracket_cycle % len(available_opponents)]

            return user_bracket[opponent_index] if opponent_index < len(user_bracket) else None
            ...
            if num_players <= 2:
                ...
                available_opponents = [i for i in range(num_players) if i != user_index]
                if len(available_opponents) == 0:
                    return None  # Bye week
                opponent_index = available_opponents[cycle_num % len(available_opponents)]

            return players[opponent_index] if opponent_index < len(players) else None
    ...
    # Ensure all players have playoff records initialized
    for player in players:
        if player not in league["playoff_records"]:
            league["playoff_records"][player] = {"wins": 0, "losses": 0, "ties": 0}

    # Process each cycle one at a time to avoid duplicates
    for cycle_num in range(max_cycles):
        
        # Get all matchups for this specific cycle
        cycle_matchups = {}
        cycle_byes = []
        
        # First pass: determine who plays who in this cycle
        for player in players:
            user_data = get_user_stats(player)
            player_matchdays = len(user_data.get("data", []))

            # Only consider players who have completed this cycle
            if player_matchdays >= (cycle_num + 1) * 3:
                opponent = get_matchup_opponent_for_cycle(player, cycle_num)
                
                if not opponent:
                    cycle_byes.append(player)
                else:
                    # Create consistent pairing (alphabetical order)
                    pair = tuple(sorted([player, opponent]))
                    if pair not in cycle_matchups:
                        cycle_matchups[pair] = True

        # Second pass: process bye weeks for this cycle
        for player in cycle_byes:
            bye_id = f"{player}_bye_cycle_{cycle_num}"
            
            # Skip if this bye week has already been processed
            if bye_id in league["recorded_byes"]:
                continue
                
            # Award exactly 1 win for bye week
            league["playoff_records"][player]["wins"] += 1
            league["recorded_byes"].append(bye_id)
            logging.info(f"BYE WEEK WIN: {player} (cycle {cycle_num})")

        # Third pass: process head-to-head matchups for this cycle
        for (player1, player2) in cycle_matchups.keys():
            matchup_id = f"{player1}_vs_{player2}_cycle_{cycle_num}"
            
            # Skip if already processed
            if matchup_id in league["recorded_matchups"]:
                continue
                
            # Verify both players completed this cycle
            player1_data = get_user_stats(player1)
            player2_data = get_user_stats(player2)
            player1_matchdays = len(player1_data.get("data", []))
            player2_matchdays = len(player2_data.get("data", []))
            
            if player1_matchdays >= (cycle_num + 1) * 3 and player2_matchdays >= (cycle_num + 1) * 3:
                # Calculate points for this cycle
                player1_points = get_cycle_points(player1, cycle_num)
                player2_points = get_cycle_points(player2, cycle_num)
                
                # Determine winner - HEAD TO HEAD ONLY
                if player1_points > player2_points:
                    league["playoff_records"][player1]["wins"] += 1
                    league["playoff_records"][player2]["losses"] += 1
                    logging.info(f"HEAD-TO-HEAD: {player1} BEATS {player2} ({player1_points} vs {player2_points}) in cycle {cycle_num}")
                elif player2_points > player1_points:
                    league["playoff_records"][player2]["wins"] += 1
                    league["playoff_records"][player1]["losses"] += 1
                    logging.info(f"HEAD-TO-HEAD: {player2} BEATS {player1} ({player2_points} vs {player1_points}) in cycle {cycle_num}")
                else:
                    # Exact tie
                    league["playoff_records"][player1]["ties"] += 1
                    league["playoff_records"][player2]["ties"] += 1
                    logging.info(f"HEAD-TO-HEAD TIE: {player1} vs {player2} (both scored {player1_points}) in cycle {cycle_num}")
...
def check_and_finish_league(league_code):
    ...
    # Check if league should finish using league-wide matchday
    if league_matchday >= matchdays_limit:
        # Determine winner based on league system
        if league.get("use_playoffs", True):
            # Playoff system: winner has most wins from top bracket
            playoff_records = league.get("playoff_records", {})
            brackets = league.get("brackets", {"top": league["players"], "bottom": []})

            if playoff_records and any(record["wins"] > 0 or record["losses"] > 0 or record["ties"] > 0 for record in playoff_records.values()):
                # Winner must be from top bracket
                top_bracket_players = brackets.get("top", league["players"])
                if top_bracket_players:
                    winner = max(top_bracket_players, key=lambda x: (
                        playoff_records.get(x, {}).get("wins", 0), 
                        league_stats.get(x, 0)  # Tiebreaker: total points
                    ))
                else:
                    # Fallback if no top bracket
                    winner = max(playoff_records.keys(), key=lambda x: (
                        playoff_records[x]["wins"], 
                        league_stats.get(x, 0)
                    ))

                # Sort by bracket first, then by wins
                final_standings = []
                for bracket_name in ["top", "bottom"]:
                    bracket_players = brackets.get(bracket_name, [])
                    bracket_standings = []
                    for player in bracket_players:
                        player_record = playoff_records.get(player, {"wins": 0, "losses": 0, "ties": 0})
                        bracket_standings.append((
                            player, 
                            league_stats.get(player, 0), 
                            player_record,
                            bracket_name
                        ))
                    bracket_standings.sort(key=lambda x: (x[2]["wins"], x[1]), reverse=True)
                    final_standings.extend(bracket_standings)

                # If no brackets exist, use old logic
                ...
                final_standings = [(p, league_stats.get(p, 0), {"wins": 0, "losses": 0, "ties": 0}, "unknown") for p in league["players"]]
...
def run_automated_matchday():
    ...
    logging.info("Starting automated matchday run...")

    # Process each league
    for league_code, league in leagues.items():
        if not league.get("draft_complete"):
            logging.info(f"Skipping league {league_code} - draft not complete")
            continue

        if league.get("status") == "finished":
            logging.info(f"Skipping league {league_code} - already finished")
            continue

        logging.info(f"Processing league {league_code} with {len(league['players'])} players")

        # Advance league-wide matchday first
        advance_league_matchday(league_code)

        # Run matchday for all players in this league
        for username in league["players"]:
            try:
                result = subprocess.run([sys.executable, "core.py", username, league_code], 
                                      check=True, capture_output=True, text=True)
                logging.info(f"Successfully ran matchday for {username} in league {league_code}")
                if result.stdout:
                    logging.debug(f"Output for {username}: {result.stdout}")
            except subprocess.CalledProcessError as e:
                logging.error(f"[ERROR] Exception occurred while running core.py for {username}: {e}")
                if e.stderr:
                    logging.error(f"Error output: {e.stderr}")
                continue

        # After all players in league have played, update playoff records
        update_playoff_records(league_code)

        # Update brackets if we've reached the halfway point
        if league.get("use_playoffs", True):
            update_league_brackets(league_code)

        # Check if league should finish
        check_and_finish_league(league_code)

# Schedule automated matchdays to run every 2 minutes for testing
scheduler.add_job(
    func=run_automated_matchday,
    trigger=IntervalTrigger(minutes=2),
    id='automated_matchday',
    name='Run automated matchday for all leagues',
...
def index():
    ...
    # Add playoff records to league leaderboard if it's a playoff league
    if current_league and current_league.get("use_playoffs", True) and league_leaderboard:
        # Update playoff records before displaying
        update_playoff_records(current_league["code"])
        # Reload leagues to get updated records
        updated_leagues = load_leagues()
        updated_league = updated_leagues.get(current_league["code"], current_league)
        playoff_records = updated_league.get("playoff_records", {})

        # Add playoff records to each player in the leaderboard
        for user_entry in league_leaderboard:
        ...
        # Save updated playoff records back to leagues
        updated_leagues[current_league["code"]] = updated_league
        save_leagues(updated_leagues)
    ...
    if current_league and current_league.get("use_playoffs", True) and current_league.get("draft_complete"):
        ...
        # Get user's bracket information
        if "brackets" in current_league:
            brackets = current_league["brackets"]
            if username in brackets.get("top", []):
                user_bracket = "top"
            elif username in brackets.get("bottom", []):
                user_bracket = "bottom"
...
def run_matchday():
    ...
    if not current_league:
        flash("You must be in a league to run a matchday.", "warning")
        return redirect(url_for("index"))

    if not current_league.get("draft_complete"):
        flash("Draft must be completed before running matchdays.", "warning")
        return redirect(url_for("index"))

    # Advance league-wide matchday
    advance_league_matchday(current_league["code"])

    # Run matchday for all players in the league
    try:
        for player in current_league["players"]:
            subprocess.run([sys.executable, "core.py", player, current_league["code"]], check=True)

        flash("Matchday completed successfully for all players!", "success")

        # Update playoff records after matchday
        update_playoff_records(current_league["code"])

        # Update brackets if we've reached the halfway point
        if current_league.get("use_playoffs", True):
            update_league_brackets(current_league["code"])

        # Check if league should finish
        check_and_finish_league(current_league["code"])

    except subprocess.CalledProcessError as e:
        flash(f"Error running matchday: {e}", "danger")
    ...
    if request.method == "POST":
        ...
        if action == "create":
            ...
            leagues[code] = {
                ...
                "playoff_records": {username: {"wins": 0, "losses": 0, "ties": 0}} if use_playoffs else {},
                "recorded_matchups": []
            }
            ...
            if join_code in leagues:
                league = leagues[join_code]
                if username not in league["players"]:
                    league["players"].append(username)
                    # Initialize playoff record for new player
                    if league.get("use_playoffs", True):
                        if "playoff_records" not in league:
                            league["playoff_records"] = {}
                        league["playoff_records"][username] = {"wins": 0, "losses": 0, "ties": 0}
                    save_leagues(leagues)
...
def api_matchup_points(username):
    ...
    # Get user's league to use league-wide matchday
    current_league = get_user_league(username)
    if not current_league:
        return jsonify({"points": 0}), 401

    league_matchday = current_league.get("current_matchday", 0)

    # Calculate which 3-game cycle we're currently in
    current_cycle = league_matchday // 3
